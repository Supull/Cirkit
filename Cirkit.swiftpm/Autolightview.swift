...
struct AutoLightView: View {
    ...
    let steps = [
        ...
        "Drag the microcontroller to connect it to the circuit like shown in the target circuit. Then double tap it to open the popup and click </> Program, then press \"Run\". Switch off the lights in your room and notice how the bulbs will light up."
    ]
        ...
        ZStack {
            ...
            VStack {
                ...
                VStack(spacing: 8) {
                    ...
                    Button("Light Sensor") {
                        spawnEntity(fileName: "LightSensor")
                    }
                }
            ...
            VStack(spacing: 0) {
                Spacer().frame(height: 280) // Top spacing below banner + circuit progress
                ...
                Spacer() // This pushes everything to top, doesn't affect other UI
            }
            .frame(maxHeight: .infinity, alignment: .top)
            .zIndex(500)
            ...
            if showMicrocontrollerPopup, let mcEntityName = selectedMicrocontrollerEntity {
                ...
                MicrocontrollerInfoPopup(
                    ...
                    onPortStateChanged: { port, newState in
                        connectionManager.setMicrocontrollerPortState(mcName: mcEntityName, port: port, isHigh: newState)
                    },
    ...
    func spawnPrebuiltCircuit() {
        ...
        loadAndPlace(fileName: "MicrocontrollerM", xPos: -0.019, zPos: -0.18, rotationY: .pi * 2)
        
        loadAndPlace(fileName: "Battery5",          xPos:  0.340, zPos:  0.037)
        loadAndPlace(fileName: "Wire4",             xPos:  0.168, zPos:  0.036)
        loadAndPlace(fileName: "Wire4",             xPos: -0.121, zPos:  0.011)
        loadAndPlace(fileName: "Wire4",             xPos: -0.206, zPos:  0.061)
        loadAndPlace(fileName: "Wire4",             xPos:  0.086, zPos:  0.106)
        loadAndPlace(fileName: "Bulb",              xPos: -0.253, zPos:  0.011, rotationY: .pi/2)
        loadAndPlace(fileName: "Bulb",              xPos: -0.337, zPos:  0.061, rotationY: .pi/2)
        loadAndPlace(fileName: "LightSensor",       xPos:  0.188, zPos:  0.121)  // No rotation override - uses default
